...
/**
 * Attempts to test the {@link SimpleReadWriteLock} and {@link ThreadSafeIndexedSet}. These tests
 * are not 100% accurate. They attempt to create threads in such a way that problems will occur if
 * the implementation is incorrect, but the tests are inexact.
 */
...
public class SimpleReadWriteLockTest {
  ...
  /**
   * Tests that two threads are able to simultaneously acquire the read lock without any exceptions.
   * Should also finish in less than 200 milliseconds if both threads are able to execute
   * simultaneously.
   *
  ...
  /**
   * Tests that two threads are NOT able to simultaneously acquire the write lock without any
   * exceptions. Should also finish in over 200 milliseconds if both threads are able to execute
   * simultaneously.
   *
   * @throws InterruptedException if interrupted
   */
  ...
  /**
   * Tests that two threads are NOT able to simultaneously acquire the read lock and write lock
   * without any exceptions. Should also finish in over 200 milliseconds if both threads are able to
  ...
  /**
   * Tests that two threads are NOT able to simultaneously acquire the read lock and write lock
   * without any exceptions, but multiple threads may acquire read locks (even if a writer is
   * waiting). Should also finish in over 200 milliseconds if all threads are able to execute
   * properly.
   *
   * @throws InterruptedException if interrupted
   */
  ...
  /**
   * Tests that calling unlock() without first calling lock() for a write lock throws a
  ...
  /**
   * Tests that calling unlock() twice for a single lock() call for a write lock throws a
   * {@link ConcurrentModificationException}.
   */
  @Test
  @Order(8)
